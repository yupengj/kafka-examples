...
public class KafkaStreamJoin {
    ...
    public static void main(String[] args) {
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "kafka-stream-merge-ext-table-test-1"); // 全局唯一
        ...
        final String toTopic = "ibom-test.mstdata.md_change";
        ...
        }, Materialized.with(Serdes.String(), mergedValueSerde)).toStream().map((key, value) -> {
            final ObjectNode objectNode = (ObjectNode) value;
            if (objectNode.has("createDate")) {
                objectNode.replace("createDate", convertTimestamp(objectNode.get("createDate")));
            }
